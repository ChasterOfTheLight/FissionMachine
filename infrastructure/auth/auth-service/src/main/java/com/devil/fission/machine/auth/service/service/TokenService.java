...
public class TokenService {
    ...
    public TokenDto generateToken(TokenEntity tokenEntity, long expireTimeSeconds) {
        ...
        if (!isKicked) {
            ...
            tokenKey = tokenKey + ":" + IdUtil.fastUUID();
        }
        ...
        // 加锁缓存token
        RLock lock = redissonClient.getLock(lockPrefix + tokenEntity.getLoginPlatform().name() + ":" + tokenEntity.getUserId());
        try {
    ...
    public Response<VerifyTokenDto> verifyToken(String requestToken) {
        ...
        if (StringUtils.isNotEmpty(requestToken)) {
            ...
            // 查询缓存，看是否有分布式session
            String tokenEntityInRedis = redisService.getAsString(tokenPrefix + platform + ":" + tokenKey);
            if (tokenEntityInRedis == null) {
                // 刷新token如果有并发，可能会有旧token的请求，但此时已经更换新token，导致旧token在缓存中找不到，需要放行这部分请求
                if (!redisService.hasKey(lockPrefix + refreshTokenPrefix + platform + ":" + tokenKey)) {
            ...
            try {
                ...
                if (isRefreshToken) {
                    // 在刷新token白名单中直接处理
                    String whiteListToken = redisService.getAsString(tokenWhileListPrefix + platform + ":" + tokenKey);
    ...
    private String refreshToken(String requestToken) {
        ...
        // 防并发加锁
        RLock lock = redissonClient.getLock(lockPrefix + refreshTokenPrefix + platform + ":" + tokenKey);
        try {
            boolean locked = lock.tryLock(100L, 1000L, TimeUnit.MILLISECONDS);
            if (locked) {
                ...
                redisService.setEx(tokenWhileListPrefix + platform + ":" + tokenKey, requestToken, 2000L, TimeUnit.MILLISECONDS);
                // 判断是否正常塞入
                if (!redisService.hasKey(cacheKey) || !redisService.hasKey(tokenWhileListPrefix + platform + ":" + tokenKey)) {
                    // 返回请求token重试
                    return requestToken;
                }
                return newToken;
    ...
    private String buildCacheKey(String tokenKey, String platform) {
        return tokenPrefix + platform + ":" + tokenKey;
    }
