    ...
    public GatewayFilter apply(Config config) {
        return new GatewayFilter() {
            ...
            public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
                ...
                String v2Path = "/v2/api-docs";
                String v3Path = "/v3/api-docs";
                boolean swaggerPath = (path.endsWith(v2Path) || path.endsWith(v3Path));
                // 判断是swagger的才处理
                if (swaggerPath) {
                    String folderSeparator = "/";
                    String[] originalParts = StringUtils.tokenizeToStringArray(path, folderSeparator);
                    
                    // all new paths start with
                    StringBuilder newPath = new StringBuilder(folderSeparator);
                    ...
                    if (newPath.length() > 1 && path.endsWith(folderSeparator)) {
                        newPath.append('/');
                    }
