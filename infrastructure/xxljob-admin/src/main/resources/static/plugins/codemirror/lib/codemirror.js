...
(function (global, factory) {
    ...
    if (old) {
      for (var i = 0; i < old.length; ++i) {
        ...
        if (endsAfter || span.from == endCh && marker.type == "bookmark"
          ...
          ;(nw || (nw = [])).push(
              new MarkedSpan(marker, startsBefore ? null : span.from - endCh,
  ...
  function collapsedSpanAround(line, ch) {
    ...
    if (sps) {
      for (var i = 0; i < sps.length; ++i) {
        var sp = sps[i];
        if (sp.marker.collapsed && (sp.from == null || sp.from < ch) && (sp.to
                == null || sp.to > ch) &&
            (!found || compareCollapsedMarkers(found, sp.marker) < 0)) {
          found = sp.marker;
        }
    ...
    if (sps) {
      for (var i = 0; i < sps.length; ++i) {
        ...
        var fromCmp = cmp(found.from, from) || extraLeft(sp.marker) - extraLeft(
            marker);
        var toCmp = cmp(found.to, to) || extraRight(sp.marker) - extraRight(
            marker);
  ...
  function getContextBefore(cm, n, precise) {
    ...
    var context = saved ? Context.fromSaved(doc, saved, start) : new Context(
        doc, startState(doc.mode), start);
  ...
  function splitSpaces(text, trailingBefore) {
    ...
    for (var i = 0; i < text.length; i++) {
      ...
      if (ch == " " && spaceBefore && (i == text.length - 1 || text.charCodeAt(
          i + 1) == 32)) {
        ch = "\u00a0";
      }
      result += ch;
  ...
  function measureCharInner(cm, prepared, ch, bias) {
    ...
    if (node.nodeType == 3) { // If it is a text node, use a range to retrieve the coordinates.
      for (var i$1 = 0; i$1 < 4; i$1++) { // Retry a maximum of 4 times when nonsense rectangles are returned
        ...
        while (place.coverStart + end < place.coverEnd && isExtendingChar(
      ...
      var rects;
      if (cm.options.lineWrapping && (rects = node.getClientRects()).length
          > 1) {
    ...
    if (boxAround) {
      ...
      sticky = ch == 0 ? "after" : ch == lineObj.text.length ? "before" :
          (measureCharPrepared(cm, preparedMeasure, ch - (ltr ? 1 : 0)).bottom
              + widgetHeight$$1 <= y) == ltr ?
              "after" : "before";
      // Now get accurate coordinates for this place, in order to get a
      // base X position
      var coords = cursorCoords(cm, Pos(lineNo$$1, ch, sticky), "line", lineObj,
          preparedMeasure);
      baseX = coords.left;
  ...
  function posFromMouse(cm, e, liberal, forRect) {
    ...
    if (forRect && coords.xRel == 1 && (line = getLine(cm.doc,
      ...
      var colDiff = countColumn(line, line.length, cm.options.tabSize)
          - line.length;
      coords = Pos(coords.line, Math.max(0,
  ...
  function drawSelectionRange(cm, range$$1, output) {
    ...
    function drawForLine(line, fromArg, toArg) {
      ...
      function wrapX(pos, dir, side) {
        ...
        var ch = side == "after" ? extent.begin : extent.end - (/\s/.test(
            lineObj.text.charAt(extent.end - 1)) ? 2 : 1);
        return coords(ch, prop)[prop]
      }

      var order = getOrder(lineObj, doc.direction);
      iterateBidiSections(order, fromArg || 0, toArg == null ? lineLen : toArg,
  ...
  function maybeScrollWindow(cm, rect) {
    ...
    if (doScroll != null && !phantom) {
      var scrollNode = elt("div", "\u200b", null,
          ("position: absolute;\n                         top: " + (rect.top
                  - display.viewOffset - paddingTop(cm.display))
              + "px;\n                         height: " + (rect.bottom
  ...
  function endOperation_W2(op) {
    ...
    if (op.adjustWidthTo != null) {
      ...
      if (op.maxScrollLeft < cm.doc.scrollLeft) {
        setScrollLeft(cm,
            Math.min(cm.display.scroller.scrollLeft, op.maxScrollLeft), true);
      }
  ...
  function updateDoc(doc, change, markedSpans, estimateHeight$$1) {
    ...
    if (change.full) {
      ...
      if (text.length == 1) {
        update(firstLine,
            firstLine.text.slice(0, from.ch) + lastText + firstLine.text.slice(
                to.ch), lastSpans);
      } else {
      ...
      update(firstLine,
          firstLine.text.slice(0, from.ch) + text[0] + lastLine.text.slice(
              to.ch), spansFor(0));
  ...
  function addChangeToHistory(doc, change, selAfter, opId) {
    ...
    if ((hist.lastOp == opId ||
            ...
            ((change.origin.charAt(0) == "+" && hist.lastModTime > time - (doc.cm
  ...
  function markText(doc, from, to, options, type) {
    ...
    if (marker.addToHistory) {
      addChangeToHistory(doc, {from: from, to: to, origin: "markText"}, doc.sel,
          NaN);
    }
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    addSelection: docMethodOp(function (anchor, head, options) {
      ...
      setSelection(this, normalizeSelection(this.cm, ranges, ranges.length - 1),
          options);
    }),
    ...
    findMarks: function (from, to, filter) {
      ...
      this.iter(from.line, to.line + 1, function (line) {
        ...
        if (spans) {
          for (var i = 0; i < spans.length; i++) {
            var span = spans[i];
            if (!(span.to != null && lineNo$$1 == from.line && from.ch
                    >= span.to ||
    ...
    linkedDoc: function (options) {
      ...
      var copy = new Doc(getLines(this, from, to),
          options.mode || this.modeOption, from, this.lineSep, this.direction);
  ...
  function bidiSimplify(cm, range$$1) {
    ...
    var boundary = index + ((part.from == anchor.ch) == (part.level != 1) ? 0
        : 1);
    ...
    if (head.line != anchor.line) {
      leftSide = (head.line - anchor.line) * (cm.doc.direction == "ltr" ? 1
          : -1) > 0;
    } else {
    ...
    return anchor.ch == ch && anchor.sticky == sticky ? range$$1 : new Range(
  ...
  function registerEventHandlers(cm) {
    ...
    on(d.scroller, "touchstart", function (e) {
      if (!signalDOMEvent(cm, e) && !isMouseLikeTouchEvent(e) && !clickInGutter(
          cm, e)) {
  ...
  function applyTextInput(cm, inserted, deleted, sel, origin) {
    ...
    for (var i$1 = sel.ranges.length - 1; i$1 >= 0; i$1--) {
      ...
      var changeEvent = {
        ...
        origin: origin || (paste ? "paste" : cm.state.cutIncoming ? "cut"
            : "+input")
      };
