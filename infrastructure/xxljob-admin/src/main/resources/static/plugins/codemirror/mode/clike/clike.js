...
(function (mod) {
  ...
  CodeMirror.defineMode("clike", function (config, parserConfig) {
    ...
    return {
      ...
      token: function (stream, state) {
        ...
        if (curPunc == ";" || curPunc == ":" || (curPunc == "," && stream.match(
            /^\s*(?:\/\/.*)?$/, false))) {
  ...
  def(["text/x-c++src", "text/x-c++hdr"], {
    name: "clike",
    keywords: words(
        cKeywords + " dynamic_cast namespace reinterpret_cast try explicit new "
        +
    ...
    hooks: {
      ...
      "=": function (stream, state) {
        ...
        if (cx.type == "}" && cx.align && stream.eat(">")) {
          state.context = new Context(cx.indented, cx.column, cx.type, cx.info,
              null, cx.prev)
          return "operator"
        } else {
          return false
    ...
    hooks: {
      ...
      indent: function (state, ctx, textAfter, indentUnit) {
        ...
        if (ctx.align && ctx.type == "}") {
          return ctx.indented + (state.context.type == (textAfter || "").charAt(
              0) ? 0 : indentUnit);
        }
      }
    },
