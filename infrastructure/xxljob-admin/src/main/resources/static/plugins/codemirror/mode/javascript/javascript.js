...
(function (mod) {
  ...
  CodeMirror.defineMode("javascript", function (config, parserConfig) {
    ...
    function tokenBase(stream, state) {
      ...
      if (ch == '"' || ch == "'") {
        ...
        if (state.lastType != ".") {
          ...
          if (word == "async" && stream.match(/^(\s|\/\*.*?\*\/)*[\[\(\w]/,
              false)) {
    ...
    function statement(type, value) {
      ...
      if (type == "if") {
        if (cx.state.lexical.info == "else" && cx.state.cc[cx.state.cc.length
        - 1] == poplex) {
          cx.state.cc.pop()();
        }
      ...
      if (type == "variable") {
        ...
        } else if (isTS && (value == "module" || value == "enum" || value
            == "type") && cx.stream.match(/^\s*\w/, false)) {
          cx.marked = "keyword"
          if (value == "enum") {
            return cont(enumdef);
          } else if (value == "type") {
            return cont(typeexpr, expect("operator"),
                typeexpr, expect(";"));
          } else {
            return cont(pushlex("form"), pattern, expect("{"), pushlex("}"),
                block, poplex, poplex)
          }
      ...
      if (type == "switch") {
        return cont(pushlex("form"), parenExpr, expect("{"),
            pushlex("}", "switch"), pushblockcontext,
    ...
    function expressionInner(type, value, noComma) {
      if (cx.state.fatArrowAt == cx.stream.start) {
        ...
        if (type == "(") {
          return cont(pushcontext, pushlex(")"),
              commasep(funarg, ")"), poplex, expect("=>"), body, popcontext);
      ...
      if (type == "(") {
        return cont(pushlex(")"), maybeexpression, expect(")"),
            poplex, maybeop);
      }
    ...
    function maybeoperatorNoComma(type, value, noComma) {
      ...
      if (type == "operator") {
        ...
        if (isTS && value == "<" && cx.stream.match(/^([^>]|<.*?>)*>\s*\(/,
            false)) {
    ...
    function functiondef(type, value) {
      ...
      if (type == "(") {
        return cont(pushcontext, pushlex(")"),
            commasep(funarg, ")"), poplex, mayberettype, statement, popcontext);
      }
      if (isTS && value == "<") {
        return cont(pushlex(">"),
    ...
    function classBody(type, value) {
      if (type == "async" ||
          (type == "variable" &&
              (value == "static" || value == "get" || value == "set" || (isTS
                  && isModifier(value))) &&
    ...
    return {
      ...
      token: function (stream, state) {
        ...
        state.lastType = type == "operator" && (content == "++" || content
            == "--") ? "incdec" : type;
      ...
      indent: function (state, textAfter) {
        ...
        if (type == "vardef") {
          ...
          == "operator" || state.lastType == "," ? lexical.info.length + 1 : 0);
